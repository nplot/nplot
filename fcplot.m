...
callback_linscale = 'plotstruct = guidata(gcf); plotstruct.linlog = ''LIN''; guidata(gcf,plotstruct); doplot(plotstruct,''keepview'');' ; 
callback_logscale = 'plotstruct = guidata(gcf); plotstruct.linlog = ''LOG''; guidata(gcf,plotstruct); doplot(plotstruct,''keepview''); if min(plotstruct.datalist.valuelist(:,1))<=0, fprintf(''Note: to plot on logarithmic scale, data have been shifted to positive range by adding a constant.\n''); end' ; 
callback_direct =   'plotstruct = guidata(gcf); plotstruct.presentation = ''voronoi''; guidata(gcf,plotstruct); doplot(plotstruct,''keepview'');' ; 
callback_interp =   'plotstruct = guidata(gcf); plotstruct.presentation = ''linear''; guidata(gcf,plotstruct); doplot(plotstruct,''keepview'');' ; 
callback_grid = 'plotstruct = guidata(gcf); plotstruct.showcells = ~plotstruct.showcells; guidata(gcf,plotstruct); doplot(plotstruct,''keepview'');' ;
callback_coord = 'plotstruct = guidata(gcf); plotstruct.showcoordpoints = ~plotstruct.showcoordpoints; guidata(gcf,plotstruct); doplot(plotstruct,''keepview'');' ;
callback_vectors = 'plotstruct = guidata(gcf); plotstruct.showvectors = ~plotstruct.showvectors; guidata(gcf,plotstruct); doplot(plotstruct,''keepview'');' ;
callback_hklgrid = 'plotstruct = guidata(gcf); plotstruct.showgrid = ~plotstruct.showgrid; guidata(gcf,plotstruct); doplot(plotstruct,''keepview'');' ;
callback_save = 'plotstruct = guidata(gcf); [file,path] = uiputfile(''*.*'',''Save 2D Flatcone data''); if file~=0, sdat=[plotstruct.coordlist, plotstruct.datalist.valuelist]; save([path,file],''sdat'',''-ascii''); end' ;

callback_scansave = 'plotstruct = guidata(gcf); [file,path] = uiputfile(''*.*'',''Save Flatcone scan data (simple format)''); if file~=0, sdat=[plotstruct.scandata.x, plotstruct.scandata.y, plotstruct.scandata.dy]; save([path,file],''sdat'',''-ascii''); end' ;
callback_np = 'plotstruct = guidata(gcf); np = inputdlg(''Enter number of scan points'',''Scan definition'',1,{num2str(plotstruct.scandef.np)}); if ~isempty(np), plotstruct.scandef.np = str2num(np{1}); end; guidata(gcf, plotstruct); if isfield(plotstruct,''scandata''), doscanplot(plotstruct); end ' ;
...
for f=[menuentry, contextmenu]
    ...
    uimenu(f,'Label','Interpolated','Callback', callback_interp);                   % Interpolated smooth plot
...
annotation('Textbox','Units', 'pixels','Position',[1,1,550,25], 'String', ['Norm: ' normalizeto '=' num2str(normval)], ...
    ...
    function showpos(src,evnt)        
        ...
        elseif strcmpi(plotstruct.type, 'QXY')
            if cp(1,1)>xrange(1) && cp(1,1)<xrange(2) && cp(1,2)>yrange(1) && cp(1,2)<yrange(2)
                ...
                [H, K, L] = calcHKL( cp(1,1), cp(1,2), plotstruct.datalist.QVERT, UB );
                set(plotstruct.poshandle, 'String', {['qxy = [', num2str(cp(1,1),'%5.3f'), ', ', num2str(cp(1,2),'%5.3f'), ']    '] , [ 'HKL = ' num2str(H,'%6.3f') ', ' num2str(K,'%6.3f') ', ' num2str(L,'%6.3f') '    ' ] } );
            else 
                set(plotstruct.poshandle, 'String', '');
            end
        ...
        elseif strcmpi(plotstruct.type, 'HKLvectors')
            if cp(1,1)>xrange(1) && cp(1,1)<xrange(2) && cp(1,2)>yrange(1) && cp(1,2)<yrange(2)
                ...
                qq = UB*hkl(:);
                set(plotstruct.poshandle, 'String', {['qxy = [', num2str(qq(1),'%5.3f'), ', ', num2str(qq(2),'%5.3f'), ']    '] , [ 'HKL = ' num2str(hkl(1),'%6.3f') ', ' num2str(hkl(2),'%6.3f') ', ' num2str(hkl(3),'%6.3f') '    ' ] } );
            else 
                set(plotstruct.poshandle, 'String', '');
            end
   ...
   function mouseclick1(src,evnt)
     ...
     if strcmp(get(gcf,'SelectionType'),'normal')
        ...
        function mouseup1(src,evnt)
           ...
           plotstruct.scandef.np = estimateNP([plotstruct.scandef.xdat', plotstruct.scandef.ydat'], plotstruct.vertexlist, plotstruct.datalist.faces);
...
%% Callback routines for movement, rotation and resize of projection 
    ...
    function mouseclick_projectline(src,event)
        if strcmp(get(gcf,'SelectionType'),'normal')
            plotstruct = guidata(gcf);
            % Get current mouse position
            cp = get(plotstruct.axeshandle,'CurrentPoint');
...
%% Subfunction: Create a set of scans from multiple surfaces in 3D
    ...
    function createscanset
        ...
        newdata = plotstruct.datalist; 
        newdata.coordlist = [];
        newdata.valuelist = [];
        newdata.expname = plotstruct.datalist.expname;
        ...
        tempplotstruct.datalist.vertexlist = tempplotstruct.datalist.vertexlist(:,1:2);
        if isfield(tempplotstruct.datalist,'delaunaytri'), tempplotstruct.datalist = rmfield(tempplotstruct.datalist,'delaunaytri'); end
        tempplotstruct.scandef.np = round(tempplotstruct.scandef.np / nsecs);
        
        nonconstcount = 0;
        for nsl=1:nsecs
            
            yval = plotstruct.coordlist(section==nsl,3);
            if max(yval)-min(yval) > stdgrid(3), nonconstcount = nonconstcount+1; end
            yval = mean(yval);
            
            tempplotstruct.datalist.faces = plotstruct.datalist.faces(section==nsl,:);
            tempplotstruct.datalist.coordlist = plotstruct.datalist.coordlist(section==nsl,1:2);
            tempplotstruct.datalist.valuelist = plotstruct.datalist.valuelist(section==nsl,:);
            tempplotstruct.datalist.monitorlist = plotstruct.datalist.monitorlist(section==nsl,:);
            tempplotstruct.coordlist = plotstruct.coordlist(section==nsl,1:2);
        ...
        planedef.properties = {'FaceColor','r','FaceAlpha',.35};
...
%% Subfunction: Analyze input of Scan definition
    ...
    function qxy = translateinput(text)
        ...
        function errormessage
            errordlg('Format: In each line, give either two values in reciprocal Angstrom (for example: "qxy  -2.5  3.2") or three HKL (ex.: "hkl 2 0 0"). For q-en plots, give two values for |q| (A-1) and En (meV), "qen 1.5 2.0".'); 
        end
        ...
        for i = 1:length(text)
            ...
            switch coord
                ...
                case 'hkl'
                    ...
                    if isfield(plotstruct.datalist,'QVERT') && abs((hkl(1)*zh + hkl(2)*zk + hkl(3)*zl)-plotstruct.datalist.QVERT) > maxdeviate.QVERT  && ~qvwarning
...
%% Callback: Numerical Input of Scan Definition
    ...
    function inputscandef(src,evnt)
        ...
        if strcmpi(plotstruct.scandef.type,'Interpolation')
            try
                if plotopt.preferHKL && ~strcmpi(plotstruct.type, 'qeplane')
                    ...
                    defaulttext = {[stdcoord num2str(plotstruct.scandef.xdat(1)) '  ' num2str(plotstruct.scandef.ydat(1))],  [stdcoord num2str(plotstruct.scandef.xdat(2)) '  ' num2str(plotstruct.scandef.ydat(2))] };
                end
            catch
                defaulttext = {'', ''}; 
            end
        ...
        plotstruct.scandef.np = estimateNP([plotstruct.scandef.xdat', plotstruct.scandef.ydat'], plotstruct.vertexlist(:,1:2), plotstruct.datalist.faces);
        ...
        elseif any(strcmpi(plotstruct.scandef.type,{'Projection','Integration'}))
            ...
            plotstruct.projectframehandle = line('XData', [plotstruct.scandef.xdat(1)+framevec(1), plotstruct.scandef.xdat(2)+framevec(1), plotstruct.scandef.xdat(2)-framevec(1), plotstruct.scandef.xdat(1)-framevec(1), plotstruct.scandef.xdat(1)+framevec(1)], ...
                                                 'YData', [plotstruct.scandef.ydat(1)+framevec(2), plotstruct.scandef.ydat(2)+framevec(2), plotstruct.scandef.ydat(2)-framevec(2), plotstruct.scandef.ydat(1)-framevec(2), plotstruct.scandef.ydat(1)+framevec(2)],'Marker','none','color','r');
...
%% Callback: Delete a point by clicking
    ...
    function start_clickdelete(src,evnt)  % Called by menu, sets the mouse callback routine
        ...
        if ~isfield(plotstruct.datalist,'delaunaytri')
            ...
            try
            plotstruct.datalist.delaunaytri = delaunay(stdratio(1) * plotstruct.datalist.coordlist(:,1), stdratio(2) * plotstruct.datalist.coordlist(:,2));
            catch
            plotstruct.datalist.delaunaytri = delaunay(stdratio(1) * plotstruct.datalist.coordlist(:,1), stdratio(2) * plotstruct.datalist.coordlist(:,2),{'Qt','Qbb','Qc','Qz'});
            end
        guidata(gcf,plotstruct);
        end
    ...
    function deleteclick(src,evnt)
        ...
        plotstruct = deletepoint(plotstruct,nface);
...
%% Callback: Select a region to delete
    function start_deleteregion(src,evnt)
        ...
        function selectregion(src,evnt)
            ...
            incoord = inpolygon(plotstruct.coordlist(:,1), plotstruct.coordlist(:,2), xp, yp);
            
            % delete them
            plotstruct = deletepoint(plotstruct, find(incoord));
...
%%  Callback: Change coordinate axes
    function changeaxes(src,evnt) 
        ...
        elseif strfind(upper(get(src,'Label')),'QX'), changeto = 'QXY'; if any(strcmpi(datalist.coordtype,{'AnglesEnergy','QxQyEn'})), changeto = 'QXQYEN'; end
        elseif strfind(upper(get(src,'Label')),'LATTICE'), changeto = 'hklvectors';
        end
            
        plotstruct.type = changeto;
        [ctr, opt] = coordtransform(plotstruct.datalist,changeto);
...
%% Subfunction: Save scan to file
    ...
    function savescan(src,evnt)
        ...
        fprintf(sfile, '%s\n',['# ' plotstruct.datalist.expname ', Data: ' plotstruct.datalist.dataname ]);
        fprintf(sfile, '%s\n',['# Data normalized to ' normalizeto ' = ' num2str(normval)]); 
        fprintf(sfile, '%s\n',['# Vertical momentum transfer ' num2str(plotstruct.datalist.QVERT) ' A^-1']);
        fprintf(sfile, '%s\n',['# Energy transfer ' num2str((plotstruct.datalist.KI^2-plotstruct.datalist.KF^2)*1E20*hbar^2/2/mass_n*meVJ) ' meV']);
        if any(strcmpi(plotstruct.datalist.constants, 'TEMP')),  fprintf(sfile, '%s\n',['# Temperature ' num2str(plotstruct.datalist.TEMP,4) ' K']);
        end
...
%% Subfunction: Delete a point from plotstruct
    ...
    function plotstruct = deletepoint(plotstruct,nface)
...
%         neighbors = setdiff(neighbors, nface);
        ...
        stdratio = stdratio.(upper(plotstruct.datalist.coordtype));
        maxsize = stdcell.(upper(plotstruct.datalist.coordtype));

        neighbors = findneighbors(nface, plotstruct.datalist.coordlist, plotstruct.datalist.delaunaytri);
...
%             ne_neighbors = find (sum(eqv,2) >= 2);
            ...
            ne_neighbors = findneighbors(neighbors, plotstruct.datalist.coordlist, plotstruct.datalist.delaunaytri);
            ...
            [repairvertexlist,repairfaces,repairdelaunay] = ... 
                makevoronoi( plotstruct.datalist.coordlist([neighbors;ne_neighbors],1:2), [maxsize(1), maxsize(2)] ,  stdratio, 'calctri');
...
%             % diffvertices now contains the newly appeared nodes.
            ...
            repairfaces = repairfaces + size(plotstruct.vertexlist,1); 
            
            % Replace changed faces (neighbors)
            repairfaces(:, (end+1):size(plotstruct.datalist.faces,2) ) = NaN; % fill NaNs to overwrite existing entries
            plotstruct.datalist.faces(neighbors, 1:size(repairfaces,2)) = repairfaces;
            plotstruct.datalist.faces(plotstruct.datalist.faces==0) = NaN;
            % Append new vertices
            plotstruct.datalist.vertexlist = [plotstruct.datalist.vertexlist(:,1:2); repairvertexlist]; % ** What to do here in case of evtl. more Dim's???  **
            % Transform this list
            ctr = coordtransform(plotstruct.datalist,plotstruct.type);
            plotstruct.vertexlist = ctr.vertexlist;
            
            % Optimize patch (necessary, as in case of non-unique vertices neighbors may not be recognized)
            [plotstruct.datalist.vertexlist, plotstruct.datalist.faces, vassign] = optimizepatch(plotstruct.datalist.vertexlist, plotstruct.datalist.faces);
            ...
            % Delete all triangles that contain a deleted point:
            plotstruct.datalist.delaunaytri = plotstruct.datalist.delaunaytri( ~any(ismember(plotstruct.datalist.delaunaytri, nface),2), :);

            % Add new ones:
            repairdelaunay = neighbors(repairdelaunay);   % convert delaunaytri to indices into real coordlist
            plotstruct.datalist.delaunaytri = [plotstruct.datalist.delaunaytri; repairdelaunay];
...
%                 plotstruct.datalist.delaunaytri = delaunay(stdratio(1) * plotstruct.datalist.coordlist([1:(nface(nf)-1),(nface(nf)+1):end],1), stdratio(2) * plotstruct.datalist.coordlist([1:(nface(nf)-1),(nface(nf)+1):end],2));
%                 calcnewtri = true;
%             end, end
%             
...
%         end
        ...
        lineind = setdiff(1:size(plotstruct.datalist.coordlist,1), nface);
        
        linenum = nan(size(plotstruct.datalist.coordlist,1),1); linenum(lineind) = 1:numel(lineind);
        plotstruct.datalist.delaunaytri     = linenum(plotstruct.datalist.delaunaytri);  % Renumber delaunaytri

        % Delete nface from everywhere...  // before, store the coordinates in deletelist 
        
        if isfield(plotstruct,'deletelist'), plotstruct.deletelist = [plotstruct.deletelist; plotstruct.datalist.coordlist(nface,:)];
        else plotstruct.deletelist = plotstruct.datalist.coordlist(nface,:); end
                
        plotstruct.datalist.coordlist       = plotstruct.datalist.coordlist(lineind,:);
        plotstruct.datalist.valuelist       = plotstruct.datalist.valuelist(lineind,:);
        if isfield(plotstruct.datalist,'monitorlist')
            plotstruct.datalist.monitorlist = plotstruct.datalist.monitorlist(lineind,:);
        end
        plotstruct.datalist.faces           = plotstruct.datalist.faces(lineind,:);
        plotstruct.coordlist                = plotstruct.coordlist(lineind,:);
...
%% Callback: Choose Powder line to plot
    function show_powder(src,evnt)   % Choose and plot powder lines
        ...
        for nl = 1:numel(dvals)
            for pr = find(proc)
                dl = plotstruct.datalist;
                % obtain intersection points of line with cell boundaries
                [dl.coordlist,thislinedef.connection] = calcpowderline(plotstruct.datalist, dvals(nl), pr); 
    ...
    function callback_HKLintersect(src,evnt)
        ...
        switch upper(plotstruct.type)
            case {'QXY','QXQYEN'}
                ...
                extremepoints(:,3) = plotstruct.datalist.QVERT;
        ...
        if strcmpi(plotstruct.type,'hklvectors')
            hkl = plotstruct.vertexlist(:,1)*plotstruct.basisvectors.vector1 + plotstruct.vertexlist(:,2) * plotstruct.basisvectors.vector2;
            if isfield(plotstruct.basisvectors,'vector3') && strcmpi(plotstruct.basisvectors.type3,'hkl')
                hkl = hkl + plotstruct.vertexlist(:,3) * plotstruct.basisvectors.vector3;
            end
            ...
            vl= plotstruct.vertexlist;
            if any(strcmpi(plotstruct.type,{'QXY','QXQYEN'}))
                vl(:,3) = plotstruct.datalist.QVERT;
            end        
        ...
        for h = ceil(min(H)+.01):floor(max(H)-.01)
            [linevert, lineconn] = createintersection(plotstruct.vertexlist, plotstruct.datalist.faces, vertH - h, 2);
        ...
        for k = ceil(min(K)+.01):floor(max(K)-.01)
            [linevert, lineconn] = createintersection(plotstruct.vertexlist, plotstruct.datalist.faces, vertK - k, 2);
            hkllinedef.connection = [hkllinedef.connection; lineconn + size(hkllinedef.points,1)];
            hkllinedef.points = [hkllinedef.points; linevert];
        end
        for l = ceil(min(L)+.01):floor(max(L)-.01)
            [linevert, lineconn] = createintersection(plotstruct.vertexlist, plotstruct.datalist.faces, vertL - l, 2);
            hkllinedef.connection = [hkllinedef.connection; lineconn + size(hkllinedef.points,1)];
            hkllinedef.points = [hkllinedef.points; linevert];
        end
        if ~isfield(plotstruct,'linedef'), plotstruct.linedef =  {}; end
        plotstruct.linedef = {plotstruct.linedef{:}, hkllinedef};
        guidata(gcf,plotstruct);
        doplot(plotstruct);
    end
