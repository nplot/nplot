...
%% Try to copy data from SERDON

defaultinstrument = getoption('defaultinstrument');
instrument = readinput('from',varargin);    if isempty(instrument), instrument = defaultinstrument; end

[~, filepath] = finddata(missingfiles{1}, instrument);
if isempty(filepath)
    fprintf(['Data ' missingfiles{1} ' not found.\n']);
    return;
end

if ~any(strcmpi(varargin,'noconfirm'))
    fprintf('Copy missing data from %s.\n',filepath);
    answer = input('(Enter to continue, ''n'' to quit)\n', 's');
    if strcmpi(answer,'n'), return; end
end

for i=1:length(missingfiles)
    [success(i),msg] = copyfile([filepath filesep missingfiles{i}]);
    warning(msg);
end

fileok(~fileok) = success;

%% Old version below (no longer in use. Required direct access to intrument computer)
